...
Figures \ref{lst:file1}---\ref{lst:file5}
show the qualitative comparison in a different detailed way. In these histograms
...
with one of the considered domains and involve the best performing planners\footnote{The best among the classic planners and LPRPGP which is the competitor.} ,
...
using some admissible heuristics which are $h^\text{blind}$, $h^\text{max}$ an $h^{\text{M\&S}}$,
$h^{\text{cpdb}}$ \cite{{haslum2007domain}}.
which guarantee us the optimality of the solution found. Starting from the IPC5 domains,
...
we have generated, for each percentage of sampling preferences (excpet for 100 \%), 3 sampled
...
Trucks & 18.84 & 24.0 & 23.19 & 26.0 & n/a & 24.0 & n/a & 25 \\ \hline
