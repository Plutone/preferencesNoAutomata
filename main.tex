...
$\phi$ true, then o is a soft threat of $\sf SA_{\phi, \psi}$. 

% OPERATORE DI MINACCIA PER SOMETIME-AFTER %
\begin{mydef}\label{threato_sa1}
Given an operator $o$ and a sometime-after preference $\sf{SA}_{\phi, \psi}$ of a {\stripspp} problem, $o$ is a \textbf{threat} on $\phi$ operator of $\sf{SA}_{\phi, \psi}$ if $o$ can make true $\phi$.
\end{mydef}
...
Given an operator $o$ and a sometime-after preference $\sf{SA}_{\phi, \psi}$ of a {\stripspp} problem, $o$ is a \textbf{threat} on $\psi$ operator of $\sf{SA}_{\phi, \psi}$ if $o$ can make $\psi$ false. 
...
\begin{mydef}\label{support_sa}
Given an operator $o$ and a sometime-after preference $\sf{SA}_{\phi, \psi}$ of a {\stripspp} problem, $o$ is an \textbf{updating} 
of $\sf{SA}_{\phi, \psi}$ if $o$ can make $\psi$ false.
\end{mydef}
...
\mathcal{W}(o, P) = 
\begin{cases}
\{\textit{when} \; (\textit{cond}_{{T_{\phi}}}(o, P)) \; (\pviol)\} \;
\\\;\;\;\;\;\;\text{if}\;o\;\text{is a threat on }\phi \text{ of }P\\
\{\textit{when} \; (\textit{cond}_{{T_{\psi}}}(o, P)) \; (\pviol)\} \;
\\\;\;\;\;\;\;\text{if}\;o\;\text{is a threat on }\psi \text{ of }P\\
\{\textit{when} \; (\textit{cond}_{{S_{}}}(o, P)) \; (\psi\text{-true}, \lnot \pviol)\} \;
\\\;\;\;\;\;\;\text{if}\;o\;\text{is a support of }P\\
\{\textit{when} \; (\textit{cond}_{{U_{}}}(o, P)) \; (\lnot \psi\text{-true})\} \;
\\\;\;\;\;\;\;\text{if}\;o\;\text{is a support of }P\\
%\{
%\textit{when} \; (\textit{cond}_{{S}}(o, P)) \; (\textit{seen-}\psi),\\
%\ \ \textit{when} \; (\textit{cond}_{{T}}(o, P)) \; (\pviol)
...
\item  $\textit{cond}_{{T_{\phi}}}(o, P) = 
\{ \phi \;\mid\; \phi_i \not \in {C}_{\phi}(o), \psi\text{-is-true}\}$
...
(a) a threat of $P$ on $\phi$, (b) a threat of $P$ on $\psi$, (c) a potential support or an (d) updating
operator of $P$.
% ----------------------------
...
If we want to restrict the calculation of $\alpha_{\textit{cost}}$ to
...
\begin{figure*}[t]
\centering
\includegraphics[width=6cm]{histogrammi/rovers.png}
\label{hrovers}
\qquad
\includegraphics[width=6cm]{histogrammi/tpp.png}
\label{htpp}
