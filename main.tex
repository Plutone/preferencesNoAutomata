...
$\phi$ true, then o is threat of $\sf SA_{\phi, \psi}$ because $\psi$ could be false in $s'$. An operator
$o$ can also threat the preference if can make $\psi$ false because $\phi$ could be still true in $s'$.

% OPERATORE DI MINACCIA PER SOMETIME-AFTER %
\begin{mydef}\label{threato_sa1}
Given an operator $o$ and a sometime-after preference $\sf{SA}_{\phi, \psi}$ of a {\stripspp} problem, $o$ is a \textbf{threat} 
if $o$ can make true $\phi$ or can make false $\phi$ or both in the generated state $s'$ by its application.
\end{mydef}
...
make $\phi$ true then $o$ is a possible support of $\sf{SA}_{\phi, \psi}$ because if the
...
Given an operator $o$ and a sometime-after preference $\sf{SA}_{\phi, \psi}$ of a {\stripspp} problem, $o$ is a \textbf{neutral} for $\sf{SA}_{\phi, \psi}$ if $o$ is neither a threat nor a support of preference $\sf{SA}_{\phi, \psi}$.
\end{mydef}
...
\subsection{Conditional effects for $\mathcal{SA}$ Preferences} \label{subsect:amo_comp}

The conditional effects for a compiled operator threatening  preference $\sf{SA}_{\phi}$ preference are defined as follows.
 
\begin{mydef} \label{when_sometimeafter}

Given a preference $P = \sf{SA}_{\phi}$ and an operator $o$ that threats  $P$,
the conditional effect set $\mathcal{W}(o, P)$ in the compiled version $o'$ of $o$
(according to Definition \ref{compiledOcondeff}) is:
...
\bigwedge_{\phi_i \in TC(o, \phi)} (l_1 \lor ... \lor l_q) \text{ where }\\
...
An operator $o$ affecting a preference $P = {\sf SA}_{\phi, \psi}$ can be as
(a) a threat of $P$ or a potential support of $P$. In
case (a) the condition has to captures the fact that $o$ generates a state $s'$ where $\phi$ is true and $\psi$ is false
and then $P$ is violated. In case
(b) the condition captures the fact that $o$ generates a state $s'$ where $\phi$ is true
without requiring any condition on $\psi$ because in state where $\psi$ is true $P$ is achieved.
% ----------------------------
...
In the literature, there are two main general methods for compiling away conditional effects.  In the first method by Gazen and Knoblock's \cite{gazen97:ecp}, each plan of the compiled problem preserves the length of the corresponding plan for the original problem, but an exponential number of compiled operators are generated (in our  context $O(2^m)$ operators for each $o'$).
...
many conditional effects can be generated, making the other approach impractical. Moreover, Nebel's method can be optimised for our conditional effects because of their particular structure. In particular, DARE SPIEGAZIONE DI QUALI SONO LE PROPRIETA' STRUTTURALI DEI COND EFFECTS CHE SFRUTTIAMO.
...
with {\pddlIII} preferences \cite{coles2011lprpg}.
...
We used a selection of the best performing planners at IPC-8 and IPC-9 \cite{vallati20152014,pc9website}:
...
As benchmarks we used all (100) [CHECK!!! SONO 100??] original problems
...
and memory limits of 30 minutes (compilation time included [CHECK!!! VERO?]) and 8GB, respectively. 

The compared planners are evaluated using the IPC quality score \cite{IPC6}.
...
These heuristics used with A* algorithm guarantee the optimality of the solution
...
All the experiments about the optimality
...
considering all preferences, while those in Table \ref{tab:ipc_score-class} concern plan quality when only the specified preferences of a particular class are used for the plan evaluation. Overall the compilation approach performs better than {\lprpgp}, with six planners obtaining better total IPC scores.  The comparison considering each preference class separately shows good performance for every preference class except for soft goals.
...
Figures \ref{hrovers}  and \ref{htpp} show, for each preferences class, the planners' $\alpha_{\text{cost}}$ values obtained by adding the relative $\alpha_{\text{cost}}$ for every instance in the of these two benchmark domains. Each level of the stacked histogram
...
For TPP,  indeed looking to Figure \ref{htpp} we note that the crucial preferences in this domain are 
mainly soft goals, which are more satisfied by {\lprpgp}. The search pruning technique in  {\lamap} slightly helps
{\lama} to achieves more soft goals, but not enough to reach the performance of {\lprpgp}.
...
The results about the optimality evaluation and the comparison with the automata approach
are shown in Table \ref{coverage_admissible_heuristics}. The results
inherent to Openstacks have been excluded because it was not possible to find optimal
plans even for the simplest instances. The results about the TPP domain for the WRN apporach are missing because
...
\end{tabular}
\caption{Coverage of our and Nebel compilation scheme (WRB) on the IPC5 benchmarks set with
...
for each generated instance. The best performance are indicated in bold. ``n/a'' means
that the considered heuristic was not applicable and ``---'' means that such data
were not reported in WRB work.}
