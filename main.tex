...
$$
%$\overline{C}_{\phi}(o)$ is the set of clauses of $\phi$ such that
%$$
%C_{\phi}(o) =  \{\phi_{i} : |L(\phi_{i}) \cap Z(o)| = 0, \;i \in \{1 \ldots n\} \}.
%$$
%complementary to $C_{\phi}(o)$, i.e.,  $\overline{C}_{\phi}(o) = \{ \phi_{i} : \phi_i \not \in C_{\phi}(o), \;i \in \{1 \ldots n\} \}$.
\end{mydef}

[AG: CHECK!!! CAMBIARE NOTAZIONE PERCHE' ABBIAMO TOLTO $\overline{C}_{\phi}(o)$ e DICIAMO $o$ MAKES CERTAINLY TRUE]
...
is a threat for $\mathsf{A}_{\phi_1}$, a violation for $\mathsf{A}_{\phi_2}$ and neutral for
...
\textbf{EXAMPLE}
Consider the following operator $o = \langle \textit{Pre}(o), \textit{Eff}(o) \rangle =\langle \O, \{a, \lnot c\} \rangle$ which threats a, preference $P1 \in \mathcal{A}$ 
where $P1 = (\textit{always}\text{ }\phi^{P1})$ with $\phi^{P1} = \phi_{1}^{P1} \land \phi_{2}^{P1} = (a \lor b) \land (c \lor d) $. According to Definition \ref{when_always}, we have to define a condition
$\textit{cond}(o, P))$ which has to to check that the threatened clauses $T_{\mathcal{A}}$
...
In order to do a quality comparison we have considered two different quality metrics. The 
first one that we  used is the IPC quality score, a
...
% we used
We also considered an our quality metrics that we have denoted as $\alpha_{\text{cost}}$
which is useful for understanding what class of preferences have been achieved
how important they are to achieve a good quality plan. A description follows.

Given a planner $p$ and a task $i$ we assign, if $p$ solves $i$,
...
In particular in \ref{tab:ipc_score} we have reported the quality comparison
...
calculation (the class is indicated in the header of
each subtable).

Figures \ref{hrovers}---\ref{hstorage} instead show the $\alpha_{\text{cost}}$ comparison using
aggregating the scores by domain. In this case each bar is associated to a planner
whose value is equal to $\alpha_{\text{cost}}$
...
Note that in \ref{tab:ipc_score} we have reported the results about all the tested
planners while in Table \ref{tab:ipc_score-class} Figures \ref{hrovers}---\ref{hstorage}
we have restricted the analysis, in order to synthetize the explanation,
...
which the planning system which natively supports PDDL3 preferences.
...
\caption{IPC comparison calculated using all kinds of preferences together. {\lprpgp} is the planning system which natively support preferences, while the others are all classical planners. The considered planning system are sorted by the total IPC score.}
\label{tab:ipc_score}
\end{table}\newpage
...
each subtable by the total IPC score.
}
