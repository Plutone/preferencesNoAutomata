...
many existing {\strips} planners to immediately address planning with preferences without changing their algorithms or codef.
%
...
that the proposed approach is highly competitive with respect to current planners that
...
%Planning with preferences, also called ``over-subscription planning" in \cite{Briel-et-al:AAAI04,do-kambhampati04,smith04}, concerns the generation of plans for problems involving soft goals or soft state-trajectory constraints, that it is desired a plan satisfies, but that do not have to be satisfied. 

Soft state-trajectory constraints are temporally extended goals that in {\pddlIII} are also called preferences \cite{GHLS+09}. In planning with preferences, the quality of the solution plans depends on the soft goals and preferences that are satisfied by the plans. 

%For , a useful class of preferences than can be expressed in {\pddlIII} \cite{GHLS+09} consists of  {\em always preferences}, requiring that a certain condition should hold in {\em every} state reached by a plan. As discussed in \cite{Weld:94AA,bacKab,GHLS+09,Hplanp:aij09}, adding always preferences to the problem model can be very useful to express safety or maintenance conditions, and other desired plan properties. An simple example of such conditions is ``whenever a building surveillance robot is outside a room, all the room doors should be closed". 
%
{\pddlIII} supports an useful set of types of preferences expressed through certain modal operators, and in particular the ``qualitative preferences" of types {\em at-end} (soft goals), {\em sometime}, {\em sometime-before}, {\em at-most-once}, and {\em sometime-after}. One of the competition tracks of IPC-5 was centered on qualitative preferences, and since then several systems addressing this class of planning problems have been developed. Most of these systems represent state-trajectory constraints as automata, that are compiled into the problem operators and states or are handled by the planning algorithm.
 
In this paper, we study propositional planning with qualitative preferences through a compilation approach. 
In particular, we extend previous work on compiling soft goals \cite{percassi2017improving} and soft always goals \cite{ceriani2015planning} to a 
deal with the full set of {\pddlIII} qualitative preferences. Differently from most other systems, our method directly compiles qualitative preferences into propositional planning with action costs without using automata to represent the state-trajectory constraints, and without using numerical fluents, as commonly done in other approaches. Propositional planning with action costs is supported by many powerful planners, and the proposed compilation method allows them to immediately support (through the compiled problems) qualitative preferences with no change to their algorithms and code.

The paper also presents an experimental analysis evaluating the performance of
state-of-the-art propositional planners supporting action costs using our compilation
%approach to deal with 
of
soft state-trajectory constraints.  The results indicate that the proposed approach is highly competitive with a state-of-the-art planner that
natively supports qualitative preference, as well as with a recent (automata-based) compilation approach.
...
We start with a briefly description of the main related work; then, after the necessary preliminaries, we describe the compilation method in detail; finally, we present the experimental analysis and give the conclusions.
...
The structure of the proposed compilation was inspired by the work of Keyder and Geffner (\citeyear{kn:geffner09})
on compiling soft goals into {\strips} with action costs (in the following denoted with \stripsp). 
Keyder and Geffner's compilation scheme is considerably simpler than ours because it does not to consider the different kind of interference between actions and the kinds of {\pddlIII} preferences treated in our compilation.
...
and its variants \cite{coles2013searching}.
% \hl{where some different distance-to-go heuristics paired to a cost-to-go heuristic are exploited to promote
...
Our computation method is very different from the one of {\mipsxxl} since, rather than translating automata into new operators, the problem preferences are compiled by only modifying the domain operators, possibly creating multiple variants of them. Moreover, our compiled files only use {\stripsp}, while {\mipsxxl} also uses numerical fluents.%
% RIFERIMENTO SACRIFICATO SOLO PER GUADAGNARE SPAZIO
\footnote{Another compilation scheme using numerical fluents is considered in \cite{GHLS+09} to study the expressiveness of {\pddlIII}  (without an implementation).}

The compilation of LTL goal formulas by Cresswell and Coddington (\citeyear{CC:ECAI04}) and Rintanen (\citeyear{Rintanen:ECAI00})
handle {\em hard} temporally extended goals, instead of preferences, i.e., every specified temporally extended goal must be satisfied in a valid plan, and hence there
is no notion of plan quality referred to the amount of satisfied preferences. Rintanen's compilation considers only
single literals in the formulae (while we deal with arbitrary CNF formulas), and it appears that extending
it to handle more general formulas requires substantial new techniques.
% \cite{Rintanen:personal2015}. 
%An implementation of 
%Crosswell and Coddington's approach is unavailable, but 
%
Bayer and McIlaraith (\citeyear{BM:AAAI06}) observed that Crosswell and Coddington's approach
suffers exponential blow up problems and is less efficiently than {\hplanp}.

Other works that are related to ours are the compilation schema in \cite{ceriani2015planning}, which however supports only soft goals and always preferences, and the recent schema by Wright, Mattmueller and
Nebel (\citeyear{nebel2018}) (here abbreviated WMN), which supports a class of soft state-trajectory constraints richer than the {\pddlIII} qualitative preferences.
%
WMN compiles soft trajectory constraints into conditional effects and state dependent action costs using
$\sf{LTL}_{\text{f}}$ \cite{de2014reasoning} and automata. 
%[\textbf{NOTA:} ho tolto Buchi automata perche' nell'articolo piu' recente menzionano solo LTLf che vengono convertite in NFA ed eventualmente DFA (articolo \cite{de2014reasoning})]
% and B{\"u}chi automata. 
Besides the use of automata, other main differences are the following ones.
Our compilation has less additional fluents (at most two for each preference against one for each automaton state in WMN), and WMN use numerical fluents while in our compilation they are not used.
In WMN's ``action penalty compilation", both preference satisfaction rewards and violation penalties are considered. Rewards can be modelled as negative penalties, but this severely restricts the planners that can be used, since few of them supports negative costs; alternatively, the ``positively shifted costs" technique allows to use only zero and positive costs, but this might lead to loose optimality guarantees, since from an optimal compiled plan might not correspond to an optimal plan for the original problem.
 
 %\hl{in WMN the plan cost can be increased as well as decreased (through the use of negative costs) at any point of the the plan, while we only increase it and a the end of the plan (through the {\em forgo} actions)} [\textbf{NOTA}: nell'articolo viene anche menzionato come primo approccio
%il goal action penalty compilation nel quale il costo del piano viene realizzato alla fine per mezzo di State Action Dependent Costs, MA la versione con costi negativi e' quella per la quale viene testa hFF (piani subottimi)].%
...
An action sequence $\pi=\langle a_0, \ldots, a_m\rangle$ is applicable in a planning problem $\Pi$ if all actions $a_i$ are in $O$ and there exists a sequence of states $\langle s_0, \ldots, s_{m+1}\rangle$ such that $s_0=I$, $\textit{Pre}(a_i) \subseteq s_i$ and $s_{i+1}=s_i  \setminus 
...
The definition of plan utility for {\stripspp} is similar to the one given for {\stripsp} with soft goals by Keyder and Geffner (\citeyear{kn:geffner09}). 
...
Given an operator $o$ and a CNF formula $\phi = \phi_{1} \land ... \land \phi_{n}$, the set $C_{\phi}(o)$ of clauses of $\phi$ that $o$ makes {\bf certainly true} (independently from $s$) in $s'$  is defined as:
$$
...
Given a clause $\phi_{i} = l_1 \lor ... \lor l_{m_i}$ of $\phi$, condition $|L(\phi_{i}) \cap Z(o)| > 0$ in Definition \ref{def:trueclauses} requires that at least a literal of $\phi_i$ is in $Z(o)$, and thus that clause $\phi_i$ true in $s'$.
...
%\begin{enumerate}
% $C_{\phi}(o) \not = \O$ and
$|C_{\phi}(o)|>0$ and, for each clause $\phi_i$ of $\phi$ not in ${C}_{\phi}(o), \; \overline{L}(\phi_i) \not \subseteq Z(o)$.
% \end{enumerate}
\end{mydef}

First condition in Definition \ref{def:becometrue} requires that there exists at least a clause of $\phi$ that is certainly true in $s'$ (independently from $s$), while the second requires that the clauses that are not certainly true in $s'$ are not certainly false in $s'$.

\begin{mydef}\label{def:becomefalse}
Given an operator $o$ and a CNF formula $\phi$, we say that $o$ \textbf{can make $\phi$ false} in $s'$ (dependently from $s$) if there is a clause $\phi_i$ of $\phi$ such that
\begin{enumerate}
...
The conditions of Definition \ref{def:becomefalse} require that at least a clause of $\phi$ 
(1) has some (but not all) literals which are falsified after the execution of $o$,
(2) has not literals which are true in $s'$, and (3)
 is not already false in $s$.
\begin{comment}
The expression 
...
\section{Operator-Preference interferences}

Operators and preferences may have different kinds of interference, that we have to deal with in their compilation.
We say that an operator $o$ is {\em neutral} for a preference $P$ if its execution in a plan can not affect the satisfaction of $P$ in the state trajectory of the plan. Otherwise, depending on the preferences type of $P$,  $o$ can behave as a {\em violator}, a \textit{threat} or a {\em potential support} of $P$. Informally, a violator falsifies the preference, a threat may falsify it (depending on $s$), and a potential support may satisfies it over the full state trajectory of the plan. In the following, more formal definitions of these interferences are given for each type of preference.
...
An always preference $\sf{A}_{\phi}$ is violated if ${\phi}$ is false in any state on the plan state trajectory. Hence, if $\phi$ is false in the state $s'$ generated by an operator $o$, then $o$ is a violator of $\sf{A}_{\phi}$. 
...
\noindent
\noindent {\bf Example.} An operator $o = \langle \{\}, \{\lnot a, \lnot b\} \rangle $
% $\textit{Pre}(o) = \{\}$ and $\textit{Eff}(o) = \{\lnot a, \lnot b\}$
% = \langle \lnot a, \lnot b\rangle$ 
% such that $\textit{Pre}(o) = \{\lnot a\}$ and $\textit{Eff}(o) = \{\lnot b\}$ 
is a threat for $\mathsf{A}_{\phi_1}$, a violator of $\mathsf{A}_{\phi_2}$ and neutral for
$\mathsf{A}_{\phi_3}$ where 
$\phi_1 = (b \lor c) \land d$, $\phi_2 = a \lor b$
and $\phi_3 = d$.
...
A sometime preference $\sf{ST}_{\phi}$ is violated if $\phi$ is never true
...
$o$ is a \textbf{potential support} of $\sf{ST}_{\phi}$ if $o$ can make true $\phi$, otherwise
...
\noindent {\bf Example.} An operator $o = \langle \{\}, \{\lnot b\} \rangle$ 
% such that $\textit{Pre}(o) = \{\}$ and $\textit{Eff}(o) = \{\lnot b\}$
% = \langle \top, \lnot b \rangle$
% $\textit{Pre}(o) = \O$ and $\textit{Eff}(o) = \{\lnot b\}$ 
% $\textit{Pre}(o) = \{\}$ and $\textit{Eff}(o) = \{\lnot b\}$ 
is a potential support of $\mathsf{ST}_{\phi_1}$ and a neutral for $\mathsf{ST}_{\phi_2}$
where ${\phi_1} = (c \lor \lnot b) \land a$ and ${\phi_2} = c$.
...
if operator $o$ can make ${\psi}$ true in $s'$, 
then $o$ is a potential support of $\sf{SB}_{\phi, \psi}$. Depending on the
state $s$ where such operator is applied, it can be an actual support
or neutral for $\sf{SB}_{\phi, \psi}$.
%%%%%%%%%%%%%%%%%%%
% POTENTIAL SUPPORT
%%%%%%%%%%%%%%%%%%%
\begin{mydef}\label{sup_sb}
Given an operator $o$ and a sometime-before preference $P= \sf{SB}_{\phi, \psi}$ of a {\stripspp} problem,
$o$ is a \textbf{potential support} of $P$ if $o$ can make $\psi$ true.\\
\end{mydef}
...
If an operator $o$ can make $\phi$ true in $s'$,
then $o$ is a threat of $\sf{SB}_{\phi, \psi}$. Depending on the
state $s$ where it is applied, such operator can be a violator
or neutral for $\sf{SB}_{\phi, \psi}$.
%%%%%%%%
% THREAT
%%%%%%%%
...
of a {\stripspp} problem, $o$ is a \textbf{threat} of $P$ if $o$ can make true $\phi$.
\end{mydef}
%The set of sometime-before preferences of $\Pi$ which are threatened by the operator 
%$o$ are denoted with $T_{\mathcal{SB}}(o)$.

If an operator $o$ can make neither $\phi$ nor $\psi$ true in $s'$,
then $o$ is neutral for $\sf{SB}_{\phi, \psi}$.
\begin{mydef}
Given an operator $o$ and a sometime-before preference $P= \sf{SB}_{\phi, \psi}$ of a {\stripspp} problem,
$o$ is a \textbf{neutral} of $P$ if $o$ is not a potential support or a threat.
\end{mydef}

\noindent {\bf Example.} An operator $o = \langle \{\} ,\{b\} \rangle$ 
% $\textit{Pre}(o) = \{\}$ and $\textit{Eff}(o) = \{b\}$
is a potential support of $\mathsf{SB}_{\phi_1, \psi_1}$,
a threat of $\mathsf{SB}_{\phi_2, \psi_2}$
and a operator neutral for $\mathsf{SB}_{\phi_3, \psi_3}$ where 
$\phi_1 = c $, $\psi_1 = (a \lor b) \land d$,
$\phi_2 = (c \lor b) \land d$, $\psi_2 = e$,
$\phi_3 = d$ and $\psi_3 = e$.
...
A preference $\sf{AO}_{\phi}$ is violated if ${\phi}$ is always false or if ${\phi}$ becames
true more than once in the state trajectory. Thus, if an operator $o$ can make ${\phi}$ true in $s'$, then $o$ is a threat of $\sf{AO}_{\phi}$.
...
\noindent {\bf Example.} An operator $o = \langle \{\}, \{\lnot b\} \rangle$
% $o = \langle \top, \lnot b \rangle$ 
% such that $\textit{Pre}(o) = \O$ and $\textit{Eff}(o) = \{\lnot b\}$ 
is a potential support of $\mathsf{AO}_{\phi_1}$ 
and neutral for $\mathsf{AO}_{\phi_2}$ 
where $\phi_1 = (c \lor \lnot b) \land d$ and $\phi_2 = d$.
...
Given an operator $o$ and a sometime-after preference $P= \sf{SA}_{\phi, \psi}$ of a {\stripspp} problem, $o$ is a \textbf{threat} of $P$
...
If an operator $o$ can make $\psi$ true in $s'$, then it is a potential support of $\sf{SA}_{\phi, \psi}$ because if the
preference is temporarily violated in the plan state trajectory up to $s$, then $o$ could make it satisfied.
 
% OPERATORE DI SUPPORTO PER MINACCIA SOMETIME-AFTER %
\begin{mydef}\label{support_sa}
Given an operator $o$ and a sometime-after preference $P=\sf{SA}_{\phi, \psi}$ of a {\stripspp} problem, $o$ is a \textbf{potential support} of $P$ if $o$ can make $\psi$ true.
\end{mydef}
...
Given an operator $o$ and a sometime-after preference $P= \sf{SA}_{\phi, \psi}$ of a {\stripspp} problem, $o$ is a \textbf{neutral} for $P$  if $o$ is neither a threat nor a potential support of $P$.
\end{mydef}

%\hl{NOTA: ho messo due minaccie perche' una preferenza SA puo' essere minacciata in due modi}
\noindent {\bf Example.} An operator $o = \langle\{\}, \{\lnot a, b\} \rangle $
% $ such that $\textit{Pre}(o) = \{\}$ and $\textit{Eff}(o) = \{\lnot a, b\}$
...
$\phi_1 = c$ , $\psi_1 = \lnot a \land c$,
$\phi_2 = \lnot a \land c$, $\psi_2 = c$,
$\phi_3 = \lnot d$, $\psi_3 = \lnot b \land c $,
$\phi_4 = c$ and $\psi_4 = d$.
...
We will use $O_{\textit{neutral}}$ to denote the set of the problem operators that are neutral for all preferences, and $P_{\textit{affected}(o)}$ to denote the set of all preferences in the problem that are affected by an operator $o$.
% ------------------
% AFFECTED OPERATORS
% ------------------
\begin{comment}
...
Since the compilation of soft goals is the same as in \cite{kn:geffner09}, we omit its description, and we focus on the other types of preferences.
...
\begin{comment}
Given a {\stripspp} problem, an equivalent {\stripsp} problem can be derived by translation which has some similarities to what proposed by Keyder and Geffner for soft goals but also significant difference. The scheme proposed by Keyder and Geffner is considerable simpler than ours because it does not to consider the interference between actions and preferences such as threats, potential supports and violators. In order to simplify the compilation scheme we don't consider the compilation of soft goals because it can be easily added using the same method of Keyder and Geffner.
...
\begin{itemize}
    ...
    \item $I' = I \cup \overline{C'} \cup V_{\mathcal{ST}} \cup V_{\mathcal{SA}}  \cup S_{\mathcal{AO}} \cup \{\text{\it normal-mode}\}$;
...
\begin{itemize}
    ...
    \item$V_{\mathcal{ST}} =  \{P_i\text{\it-violated} \mid  P_i \in \mathscr{P}_\mathcal{ST}\}$; 
     ...
     \item 
    $V_{\mathcal{SA}} =  \{P_i\text{\it-violated} \mid  P_i = \mathsf{SA}_{\phi,\psi}, I \models \phi \land \neg \psi \}$;
...
The {\it collect} and {\it forgo} actions can only appear at the end of the plan. For each preference $P$ the compilation of $\Pi$ into $\Pi'$ adds a dummy hard goal $P'$ that is false in the initial state $I'$;  $P'$ can be achieved either by action $\text{\it collect}(P)$, that has cost 0 but requires $P$ to be satisfied,
...
The $P_i\textit{-violated}$ literals in the compiled initial state $I'$ are used to consider each $\mathcal{ST}$ and $\mathcal{SA}$ preference that is not satisfied in $I$ violated until an operator supporting them is inserted into the plan; the $P_i\textit{-seen}$ literals in $I'$ are necessary to capture the violation of the corresponding $\mathcal{AO}$ preference when an operator makes the preference formula true for the second time in the state trajectory.

Function $ \textit{comp}(o , \mathscr{P}) $ transforms an original operator $o$
into the equivalent compiled operator $o'$ with an additional precondition forcing it to appear before the 
...
%
The definition of such additional effects depends on the type of the affected preference, and on how $o$
interferes with it; this is detailed below.
...
Note that the original initial state $I$ is extended to $I'$ with the set of literals $V_{\mathcal{ST}}$, which contains the literal $P_i\textit{-violated}$ for each sometime preference of the problem. The literal $P_i\textit{-violated}$ states that the related preference $P_i$ is (temporarily) violated in $I$ until a support operator of $P$ is applied.
...
%\noindent
In the following subsections, $\phi_i$ denotes a clause of $\phi$, $\psi_i$ denotes a clause of $\psi$, and a set of formulas is interpreted as their conjunction.
...
For each affected preference $P= \sf{A}_{\phi}$, $o'$ has a (conditional) effect $\pviol$ with a condition
...
if $o$ is a threat, the condition
checks that there exists at least a clause of ${\phi}$ that is certainly false
in $s'$ -- this is the case if there is at least a 
threatened clause whose literals that are not falsified in $s'$
are false in $s$.

\noindent \textbf{Example.} Consider an operator $o = \langle \{\}, \{\lnot a, \lnot c\} \rangle$
% $\textit{Pre}(o) = \{\}$ and $\textit{Eff}(o) = \{\lnot a, \lnot c\}$
% =\langle \top, \{a, \lnot c\} \rangle$ 
...
a potential support $o$ of $P$ makes $\phi$ true when
 all clauses of $\phi$ not in ${C}_{\phi}(o)$
%$ { \phi_i \not \in {C}_{\phi}(o)} $ 
hold in $s$, where ${C}_{\phi}(o)$ is the set of clauses of $\phi$
that are certainly true in $s'$. If this condition holds in $s$, then $o'$ falsifies $\pviol$.
% in $s'$.
...
\mathcal{W}(o, P) = 
\begin{cases}
\{\textit{when} \; (\textit{cond}_{{S}}(o, P)) \; (\psi\textit{-seen})\} \;
...
\}
$
% \item  $\textit{eff}_{\mathcal{S}}(o, P) = \psi\textit{-seen}$ 
% \end{itemize}
% and:
% \begin{itemize}
\item  $\textit{cond}_{{T}}(o, P) = 
\{\lnot \psi\textit{-seen}\} \cup \{
\phi_i \;\mid\; \phi_i \not \in {C}_{\phi}(o) \}    
% \{\lnot \psi\textit{-seen}\} \cup \{ { \phi_i \in \overline{C}_{\phi}(o)} \}
$.
...
(b) a threat of $P$, or (c) both. These cases are captured by the two conditional effects of $o'$ in Definition \ref{when_sometimebefore}.

In case (a), if all clauses that are not certainly true in $s'$ (i.e., $\textit{cond}_{{S}}(o, P)$) hold in $s$, then $\psi$ is true in $s'$, and
$o'$ keeps track of this by making $\psi\textit{-seen}$ true.
In case (b), if $\psi$ has never been true in the state-trajectory up to $s$ and all clauses of $\phi$ that are not certainly true (i.e., $\textit{cond}_{{T}}(o, P)$) hold in $s$, then $P$ is violated by $o$ and $o'$ makes $P${\it -violated} true in $s'$. In case (c), if the conditions of both conditional effects hold, $P$ is violated because $\psi$ is made true simultaneously with $\phi$.
...
making the predicate $\psi\textit{-seen}$ true.
...
% $\textit{eff}_{\mathcal{S}}(o, P) = \{\psi\textit{-seen}\}$ is made true is $s'$.
...
The conditional effects for a compiled operator threatening a preference $\sf{AO}_{\phi}$ are defined as follows.
...
\begin{align*}
\mathcal{W}(o,&P) = \{ 
\text{when } (\text{cond}_{{N}}(o, P))\text{ }(\phi\textit{-seen}))\\
&
\;\;\;\;\;\;\;\;\;\;\;
...
where:
\begin{itemize}
\item  $\text{cond}_{{N}}(o, P) = \{\lnot \phi\textit{-seen}\} \cup \{ { \phi_i \mid \phi_i \not \in {C}_{\phi}(o)} \}$
% \item  $\text{eff}_{\mathcal{N}}(o, P) =  $
\item  
% \begin{align*}
$
\text{cond}_{{T}}(o, P) = \{\phi\textit{-seen}\} \cup \{ { \phi_i \mid \phi_i \not \in {C}_{\phi}(o)} \}\; \cup $
...
Otherwise, if (1) $\phi$ was true in any state before $s'$, (2) the execution of $o$ in $s$ makes $\phi$ true , and (3) $\phi$ was false before (i.e., the three condition sets in $\text{cond}_{{T}}(o, P)$), then $o$ violates $P$ and $o'$ makes $\pviol$ true.
...
The conditional effects for a compiled operator affecting  a preference $\sf{SA}_{\phi}$ preference are defined as follows.
...
\begin{cases}
\{\textit{when} \; (\textit{cond}_{T}(o, P)) \; (\pviol)\} \;
\\\;\;\;\;\;\;\text{if}\;o\;\text{is a threat of }\phi \text{ of }P\\
...
(a) a threat of $P$, (b) a potential support of $P$, or both. In
case (a) the effect condition captures the fact that $o$ generates a state $s'$ where $\phi$ is true and $\psi$ is false, (temporarily) violating $P$. In case
(b) the condition captures the fact that $o$ generates $s'$ in which $\psi$ is true, and so $s'$ cannot violate $P$. In case (c) only one of the two conditional effects can hold in $s'$ because their conditions are mutually exclusive.
...
It can be proved that the original {\stripspp} problem has a solution plan with a certain total cost (sum of its action costs and of the violated preference costs) if and only if the compiled plan has a solution with the same total cost.

\begin{prop}
Let $\Pi'$ be the compiled problem with conditional effects of a {\stripsp} problem $\Pi$. From any plan $\pi$ solving $\Pi$ we can derive a plan $\pi'$ solving $\Pi'$, and viceversa, such that the total costs of $\pi$ and $\pi'$ are the same.
\end{prop}
%\vspace{-0.3cm}
\begin{proof}(Sketch).
The proof has the same structure of the plan-correspondence proof for Keyder and Geffner's compilation of soft goals \cite{kn:geffner09}, with $\pi' = \langle \pi'', end, \pi''' \rangle$ in which $\pi''$ is obtained from $\pi$ by replacing the original operators with the compiled ones involving conditional effects, and the rest of $\pi'$ is defined as in Keyder and Geffner's proof ($\pi'''$ involves only {\it collect} and {\it forgo} actions). Since the conditional effects in $\pi''$ affect only the additional fluents of the compiled problem, all original operator preconditions remain satisfied in the state trajectory of $\pi''$. Moreover, by construction of the conditional effects for the compiled operators, it can be proved that in the state where $\textit{end}$ is applied, for each preference $P$, $P\textit{-violated}$ holds if and only if $P$ is violated in $\pi$. Viceversa, from a valid plan $\pi'$ we can obtain a plan for $\pi$ by replacing the compiled operators with their original version, and removing $\textit{end}$  and all {\it collect}/{\it forgo} actions.
By construction of the conditional effects in the compiled operators and of $\pi$ and $\pi'$, $\pi$ violates a preference if and only if $\pi'''$ contains the corresponding {\it forgo} action. It follows that $\pi$ and $\pi'$ have the same total costs.
\end{proof}
...
In the literature, there are two main general methods for compiling  conditional effects away.  In the first method, proposed by Gazen and Knoblock (\citeyear{gazen1997combining}), each plan of the compiled problem preserves the length of the corresponding plan for the original problem, but an exponential number of compiled operators are generated.
% (in our  context $O(2^m)$ operators for each $o'$).
In the second method, proposed by Nebel (\citeyear{nebel2000compilability}), a polynomial number of new operators are generated, but each plans for the compiled problem increases polynomially the length. 

In our context, we use Nebel's method because, depending on the operators' structure and the input preferences,
many conditional effects can be generated, making Gazen and Knoblock's method impractical. Moreover, Nebel's method can be optimised for our conditional effects because of their particular structure. Specifically, the effects of our conditional effects concerning different preferences can never conflict, while those referring to the same preference can be resolved in the compilation by imposing a particular order of evaluation for their conditions. 
% DARE SPIEGAZIONE DI QUALI SONO LE PROPRIETA' STRUTTURALI DEI COND EFFECTS CHE SFRUTTIAMO.
%
This allows us to simplify the compilation by omitting the so called ``copy-operators" of Nebel's method. Another optimisation concerns the ordering of the set of operator pairs ``activating" the conditional effects, which in Nebel's original compilation is unordered, while in our context they can be ordered as a sort of macro operators. For lack of space, in this paper we don't give a detailed description of these optimisations, which leads to a revised method for compiling conditional effect away similar to the technique described in \cite{ceriani2015planning} for always constraints, but extended to deal with every class of {\pddlIII} qualitative preferences.

Another possible optimization is the following. For a compiled action that is a violator of a preference $P$, we can assign the violation cost of $P$ to this action, instead of to the forgo action for $P$, that is removed from the compilation together with goal $P'$ in $G'$.
...
\textit{when } (\textit{cond}_{{N}}(o, {\sf AO}_{\psi}))\text{ }(\psi\textit{-seen})),\\
...
% \item 
\begin{gather*}
\textit{cond}_{{N}}(o, {\sf AO}_{\psi}) = \\\{\lnot \psi\textit{-seen}\} \cup \{ { \psi_i \mid \psi_i \not \in {C}_{\psi}(o)} \} = \\
= \{\lnot \psi\textit{-seen}\} \cup \{ \psi_2 \} = \{\lnot \psi\textit{-seen}, d \lor f\}
\end{gather*}

% \item 
\begin{gather*}
\textit{cond}_{T}(o, {\sf AO}_{\psi}) = \{\psi\textit{-seen}\} \cup \{ { \psi_i \mid \psi_i \not \in {C}_{\psi}(o)} \} \; \cup \\
\{ \bigvee_{ \psi_i \in {C}_{\psi}(o)} (\lnot l_1 \land ... \land \lnot l_q) \mid  \{l_1, ..., l_q\} = {L}(\psi_i)\} = \\
= \{ \psi\textit{-seen} \} \cup \{ \psi_2 \} \cup \{ \{ \lnot \lnot c \land \lnot e \} \mid \{\lnot c, e\} = L(\psi_1) \}  = \\
= \{\psi\textit{-seen}, d \lor f, c \land \lnot e\}
\end{gather*}

The condition $\textit{cond}_{{N}}(o, {\sf AO}_{\psi})$ to activate the predicate $\psi\textit{-seen}$
requires that the following condition hold in the state where $o$ is applied:

\begin{itemize}

\item the predicate $\psi\textit{-seen}$ has to be false which means that $\psi^{P2}$
...
\item the fluent $\psi\textit{-seen}$ has to be true and so $\psi$
...
We implemented the proposed compilation scheme,\footnote{Our compiler and experimental data will be made available.}
and we compared the performance (plan quality) 
of several propositional planners using it and of  {\lprpgp}, a the state-state-of-the art system for satisficing planning that ``natively" supports 
{\pddlIII} preferences.\footnote{For the considered benchmarks, we observed that {\lprpgp}
performs generally better the other available systems supporting {\pddlthree} preferences.
We also tested OPTIC \cite{colesoptic} and the best variant of {\lprpgp} in \cite{coles2013searching}, but often they gave runtime errors.}
% with the considered benchmarks.}
...
We used a selection of the best performing planners from IPC8 and IPC9: 
%\cite{vallati20152014,ipc9website}:
{\lama} \cite{RicLAMA}, {\mercury} \cite{MercuryIPC8},
 {\miplan} \cite{nunez2014miplan}, {\ibacop} \cite{cenamor2014ibacop}, 
{\fdss} \cite{fdss2018}, abbreviated FDSS, and {\fdremixcomplete} \cite{fdremix}, abbreviated \fdremix.
In addition, we used a recent version of {\lama}, called {\lamapruning}, exploiting admissibile
heuristic $h_\text{R}$ for testing soft goals reachability during search \cite{percassi2017improving}.

As benchmarks we used all (100) original problems
of the qualitative preference track of IPC-5 \cite{GHLS+09}, which has five domains:
% which involve always, sometime, sometime-before, at-most-once and soft goal preferences,
Rovers, TPP, Trucks, Openstacks and Storage. The problems in Storage and TPP have no 
...
was run on the original problems. All experiments
were conducted on a 2.00GHz Core Intel(R) Xeon(R) CPU E5-2620 machine with CPU-time/memory
limits of 30 minutes/8GB, respectively. 
...
We also consider another metric for plan quality evaluation, that we call
$\alpha_{\text{cost}}$:
...
$p$ found a (preference-wise) optimal plan for $i$; if 
$\alpha_{\textit{cost}}(p, i) = 1$,  $p$ found the worst plan 
...
%
Concerning optimal planning, similarly to what 
done by Wright, Mattm\"{u}ller and Nebel (WMN) (\citeyear{nebel2018}), we have tested our scheme
using Fast Downward \cite{helmert2006fast} with three admissible heuristics: $h^\text{blind}$, assigning zero to goal states and 1 to every other state,
 $h^\text{max}$ \cite{bonet2001planning}, and $h^{\text{cpdb}}$ \cite{{haslum2007domain}}.
%These heuristics used with A* algorithm guarantee the optimality of the solution found. 
%
For this analysis we generated a set of benchmarks using the same methodology of WMN.
Starting from the IPC-5 problems,  we created
simpler instances by randomly sampling subsets of the soft trajectory constraints in the original instances:
from each original instance, five additional instances are generate, each of which has, respectively, 1\%, 5\%, 10\%, 20\% and 40\% of the original
(grounded) soft trajectory constraints; the hard goals are all unchanged, if they exist.
Since the sampled instances used by WMN are not available,
...
\begin{figure*}[t]
\centering
\includegraphics[width=8.0cm]{histogrammi/rovers.png}
%\label{hrovers}
%\qquad
\includegraphics[width=8.0cm]{histogrammi/tpp.png}
%\label{htpp}
\vspace{-0.05cm}
...
\end{tabular}
% }
\caption{IPC scores of the compared planners using the original plan metrics of the IPC-5 benchmarks.
The best scores are indicated in bold.}
% calculated using all kinds of preferences together. {\lprpgp} is the planning system which natively support preferences, while the others are all classical planners. The considered planning system are sorted by the total IPC score. The best performance are indicated in bold.}
\label{tab:ipc_score}
\end{table}
%\newpage
...
LPRPG-P & 15.02 & 7.0 & 0.0 & 19.5 & 11.0 & 52.52\\ \hline
...
MIPlan & --- & 15.08 & 9.85 & 16.84 & \bf 19.32 & 61.09\\ \hline
...
LPRPG-P & 14.11 & 2.0 & 19.0 & --- & 12.0 & 47.11\\ \hline
...
LPRPG-P & 8.63 & 14.0 & 15.5 & --- & 7.0 & 45.13\\ \hline
...
\caption{
IPC scores of a selection of the tested planners whose plans are evaluated considering each kind of preferences separately. ``---'' means that no preferences 
of this class are present. The best scores are indicated in bold.
}
...
\multicolumn{7}{|c|}{$\mathscr{P}_{\mathcal{A}}$}\\ \hline 
Planner & Rovers & TPP & Trucks & Openstacks & Storage &TOTAL \\ \hline 
\lamapruning & 14.19 & \bf20.0 & 15.0 & \bf20.0 & 19.0 & \bf 88.19\\ \hline 
IBaCoP2 & \bf16.19 & \bf20.0 & 13.0 & 19.0 & 19.0 & 87.19\\ \hline 
\fdssshort & 14.19 & 17.0 & \bf18.0 & 17.83 & \bf20.0 & 87.02\\ \hline 
LAMA(2011) & 14.54 & \bf20.0 & 13.0 & \bf20.0 & 19.0 & 86.54\\ \hline 
MIPlan & 14.71 & \bf20.0 & 12.0 & 19.0 & \bf20.0 & 85.71\\ \hline 
\fdremix & 12.64 & 15.0 & 15.0 & 18.5 & 19.0 & 80.14\\ \hline 
Mercury & 13.8 & \bf20.0 & 4.0 & \bf20.0 & \bf20.0 & 77.8\\ \hline 
LPRPG-P & 14.46 & 7.0 & 0.0 & 19.5 & 11.0 & 51.96\\ \hline \hline

\multicolumn{7}{|c|}{$\mathscr{P}_{\mathcal{S}}$}\\ \hline 
Planner & Rovers & TPP & Trucks & Openstacks & Storage &TOTAL \\ \hline 
LPRPG-P & --- & \bf19.45 & 16.48 & \bf19.43 & 14.94 & \bf 70.3\\ \hline 
\fdremix & --- & 14.73 & \bf17.12 & 18.63 & 18.57 & 69.05\\ \hline 
\fdssshort & --- & 14.66 & 16.49 & 18.4 & 18.44 & 67.99\\ \hline 
\lamapruning & --- & 14.82 & 14.36 & 18.7 & 18.9 & 66.78\\ \hline 
LAMA(2011) & --- & 13.95 & 13.57 & 17.78 & 18.29 & 63.6\\ \hline 
IBaCoP2 & --- & 16.03 & 10.7 & 17.23 & 18.61 & 62.57\\ \hline 
MIPlan & --- & 15.08 & 9.85 & 16.7 & \bf19.3 & 60.92\\ \hline 
Mercury & --- & 14.83 & 7.78 & 17.39 & 18.29 & 58.29\\ \hline \hline

\multicolumn{7}{|c|}{$\mathscr{P}_{\mathcal{AO}}$}\\ \hline 
Planner & Rovers & TPP & Trucks & Openstacks & Storage &TOTAL \\ \hline 
Mercury & \bf16.61 & \bf20.0 & 19.0 & --- & \bf20.0 & \bf 75.61\\ \hline 
\fdssshort & 16.4 & 17.0 & \bf20.0 & --- & 19.0 & 72.4\\ \hline 
\lamapruning & 14.54 & 18.0 & \bf20.0 & --- & 19.0 & 71.54\\ \hline 
LAMA(2011) & 14.36 & 17.0 & \bf20.0 & --- & \bf20.0 & 71.36\\ \hline 
\fdremix & 16.37 & 16.0 & \bf20.0 & --- & 18.0 & 70.37\\ \hline 
MIPlan & 14.03 & 16.0 & 15.0 & --- & \bf20.0 & 65.03\\ \hline 
IBaCoP2 & 13.21 & 15.0 & 16.0 & --- & 19.0 & 63.21\\ \hline 
LPRPG-P & 13.42 & 1.0 & 19.0 & --- & 12.0 & 45.42\\ \hline \hline 

\multicolumn{7}{|c|}{$\mathscr{P}_{\mathcal{SB}}$}\\ \hline 
Planner & Rovers & TPP & Trucks & Openstacks & Storage &TOTAL \\ \hline 
\lamapruning & 18.25 & \bf20.0 & \bf18.0 & --- & 19.0 & \bf 75.25\\ \hline 
LAMA(2011) & 18.18 & \bf20.0 & 15.5 & --- & 18.0 & 71.68\\ \hline 
Mercury & 17.07 & \bf20.0 & 14.5 & --- & \bf20.0 & 71.57\\ \hline 
MIPlan & 18.24 & \bf20.0 & 12.0 & --- & \bf20.0 & 70.24\\ \hline 
\fdremix & 17.49 & 16.0 & 16.5 & --- & \bf20.0 & 69.99\\ \hline 
\fdssshort & 17.46 & 17.0 & 16.5 & --- & 19.0 & 69.96\\ \hline 
IBaCoP2 & \bf19.72 & \bf20.0 & 12.0 & --- & 18.0 & 69.72\\ \hline 
LPRPG-P & 8.21 & 14.0 & 15.5 & --- & 7.0 & 44.71\\ \hline \hline

\multicolumn{7}{|c|}{$\mathscr{P}_{\mathcal{ST}}$}\\ \hline 
Planner & Rovers & TPP & Trucks & Openstacks & Storage &TOTAL \\ \hline 
\lamapruning & 14.28 & 10.0 & --- & --- & \bf19.0 & \bf 43.28\\ \hline 
\fdssshort & 16.17 & 8.0 & --- & --- & \bf19.0 & 43.17\\ \hline 
IBaCoP2 & \bf16.81 & 10.0 & --- & --- & 16.0 & 42.81\\ \hline 
LAMA(2011) & 14.41 & 9.0 & --- & --- & \bf19.0 & 42.41\\ \hline 
LPRPG-P & 9.56 & \bf17.0 & --- & --- & 14.0 & 40.56\\ \hline 
...
\caption{
IPC scores of a selection of the tested planners whose plans are evaluated considering each kind of preferences separately. The best scores are indicated in bold.
}
\label{tab:ipc_score-class}
\end{table}
\end{comment}
...
considering all preferences (i.e., the original plan metrics in these benchmarks), while those in Table \ref{tab:ipc_score-class} concern plan quality when only the class of preferences indicated in each subtable is considered for the plan metric (using the original violation costs).
% of a particular class are used for the plan evaluation.

% [CHECK!! METTIAMO TUTTI I PLANNER NELLA TABELLA 2?][FP: Se avanza spazio si]
The analysis in Table \ref{tab:ipc_score-class} considers a subset of the planners in Table \ref{tab:ipc_score}.
Overall the compilation approach performs better than {\lprpgp}, with six planners obtaining better total IPC scores.  The comparison considering each preference class separately shows good performance as well for every preference class except for soft goals.
...
is penalized due to its inferior coverage (it solves only 15 instances out of 20).
The tested planners from IPC9, {\fdremix} and FDSS 2018, perform overall better than those from IPC8, and {\lamapruning} is better than everyone else (it improves the performance of LAMA in all the considered domains except
Rovers, where there is no soft goal and $h_\text{R}$ cannot be exploited by {\lamapruning}).


On the other hand, {\lprpgp} performs similarly to {\lamapruning} in Openstacks and much better than the other planners in TPP. 
The observed poor performance of the compilation approach in TPP is mainly due to presence of many soft goals. This is not very surprising since, as shown in \cite{percassi2017improving}, compiling soft goals through Keyder and Geffner's method
can sometimes give poor planning performance.
%
...
achieve better results for preferences of classes always, sometime-before and at-most-once; however, 
but this has not a crucial impact of the overall plan quality,  because, according to the plan metrics specified in these problems, violating the
 soft-goals is more costly than violating the other preferences (or equivalently they are more useful to satisfy than the other preferences).
...
For lack of space, we will focus this analysis on two selected domains: Rovers, one of the considered domains where the proposed compilation approach works better, and TPP, the only domain where we observed poor performance compare to {\lprpgp}.
Figure \ref{hrovers-htpp} shows, for each preference class, the planners' $\alpha_{\text{cost}}$ values obtained by adding the relative $\alpha_{\text{cost}}$ for every instance of the two considered benchmark domains. Each level of the stacked histograms
represents the aggregated $\alpha_{\text{cost}}$ restricted to a specific class of preferences, which indicates how much each class of the (violated) preference contributes to the total preference violation cost.

For Rovers, the IPC score gap between the classical planners and {\lprpgp} 
is mainly due to {\lprpgp}'s violation of the sometime-before preferences. Regarding other
preferences classes, the violation costs in the generated plans are similar
except for {\ibacop}. This planner satisfies more sometime-before and sometime
preferences than the others planning, and violates more at-most-once preferences,
generally obtaining better quality plans.

For TPP,  indeed Figure \ref{hrovers-htpp} shows that the most important preferences are 
the soft goals, which are better satisfied by {\lprpgp}. The search pruning technique in  {\lamapruning} exploiting $h_\text{R}$ slightly helps
...
\end{tabular}
% }
\caption{Problem coverage of our compilation and WMN's 
% ``Goal action penality compilation'' 
compilation
for the IPC-5 benchmarks set augmented with additional problems that have sampled soft-trajectory constraints. ``n/a''  means that the considered heuristic was not applicable; ``---'' means that no data
...
For each of the three considered 
% versions of Fast Downward using 
admissible heuristics, the table indicates the percentage of 
% ``Goal action penality compilation'' 
solved problems. The results are compared with those reported in \cite{nebel2018}  for the ``goal action penality compilation''.\footnote{
A more detailed experimental comparison with WMN's approach is very difficult because, at the time of writing, WMN's compiler, compiled files and solution plans are unavailable.}
%
Domain Openstacks here is not considered because no one of the considered heuristics solved any instance. 
Also note that results for TPP are missing for WMN because they are not reported in \cite{nebel2018}.

According to these results, for the considered benchmarks, the compilation approach without automata seems quite
preferable, because an higher coverage is obtained in all three considered domains. This is the case even if the machine that we used for our experiments if less powerful (CPU and memory wise) than the one used by WMN, and moreover our CPU-time limit was half of that used by WMN.
...
\section{Conclusions}
%\vspace{-0.1cm}
We have proposed a new compilation schema for solving propositional planning augmented with {\pddlIII} soft state-trajectory constraints.
% called qualitative preferences.
Our work significantly extends the original approach of Keyder and Geffner that deals only soft goals. The results of an experimental analysis show that, despite the compilation of only soft goals may be less effective than other approaches such as the LPRPG-P planner, for the considered class of soft state-trajectory constraints our compilation is quite competitive with the state-of-the-art.
% with state-of-the-art systems supporting them. 
Moreover, since the planning language of the compiled problem is very simple, many available planners can use it.
